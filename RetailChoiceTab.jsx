...
export default function RetailChoiceTab({ counts, sq, bat, applianceOverrides, monthlyActual }) {
  ...
  return (
    <div>
      ...
      {/* ==================== PHASE 1: every slider that feeds the numbers below lives here ==================== */}
      <div className="mt-6 mb-4 pt-4 border-t-2 border-zinc-200 dark:border-zinc-700">
        <p className="text-xs font-semibold uppercase tracking-wider text-zinc-400">Set your assumptions</p>
        <p className="text-xs text-zinc-400 mt-1 leading-relaxed">
          Everything in this section feeds the results further down — nothing below this point reacts to something you
          haven't seen yet.
        </p>
      </div>
      ...
      <div className="mb-5">
        ...
        <p className="text-xs text-zinc-400 mb-2 leading-relaxed">
          These are what turn your PLC cut (in kW, from Market above) into the dollar lines in the results below. Capacity
          and transmission each multiply that same kW number by a different price; retail rate doesn't touch the battery
          savings at all — it only sets the supplier's baseline margin.
        </p>
      ...
      <div className="mb-5">
        ...
        <div className="p-3 bg-zinc-100 dark:bg-zinc-800 rounded-lg space-y-3">
          ...
          <Note>
            ...
            the hardware, so there's no SLA to enforce.
          </Note>
        </div>
      </div>
      ...
      <div className="mb-5">
        ...
        <p className="text-xs text-zinc-400 mb-2 leading-relaxed">
          These sliders set the business, not the battery physics — they decide how the total below gets split, and what it
          costs to build the customer base that earns it.
        </p>
        <div className="p-3 bg-zinc-100 dark:bg-zinc-800 rounded-lg space-y-3">
          ...
          <Slider label="Accounts enrolled" value={accounts} onChange={setAccounts} min={1000} max={200000} step={1000} fmt={(v) => (v / 1000) + "k"} hint="scales the fleet numbers below" />
          <label className="flex items-center gap-2 text-sm text-zinc-500 dark:text-zinc-400">
      ...
      {/* ==================== PHASE 2: results, computed live from everything set above ==================== */}
      <div className="mt-6 mb-4 pt-4 border-t-2 border-zinc-200 dark:border-zinc-700">
        <p className="text-xs font-semibold uppercase tracking-wider text-zinc-400">Results</p>
        <p className="text-xs text-zinc-400 mt-1 leading-relaxed">
          Calculated live from everything set above — move any slider up there and everything from here down updates.
        </p>
      </div>
      ...
      <div className="mb-4"><Note>
        ...
        slider, above) is what caps LTV even when yearly margin looks fine; residential supplier churn runs high.
      </Note></div>
      ...
      <div className="mb-6">
        ...
        <p className="text-xs text-zinc-400 mt-1.5 leading-relaxed">
          ...
          mechanism explained above — which is why the implied value per kWh runs so far above any wholesale spread.{" "}
      ...
      {/* ---------------- capture, as live numbers off the Dispatch reliability sliders above ---------------- */}
      <div className="mb-6">
        ...
        <div className="mt-1"><Note tone={cliff ? "amber" : "zinc"}>
          ...
          <br /><br />
          {noCapMarket
            ? `No capacity tag here, so this P10/P90 band doesn't apply to capacity or transmission (both $0). It still matters for the scarcity hedge above, which runs off the same forecast/availability/charged chain.`
            : cliff
          ...
          {!noCapMarket && (
            <>
              ...
              calling more days than that — most forecast-flagged days are false alarms. The {candidateDays}-day figure set
              above is a planning estimate of that; the extra wear it implies isn't costed into the totals here yet.
            </>
          )}
        </Note></div>
      </div>

      {/* ---------------- market comparison: reference, ignores the Prices/Sharing inputs above ---------------- */}
      <div className="mb-6">
      ...
      <div className="mb-5">
        ...
        <div className="p-3 bg-zinc-100 dark:bg-zinc-800 rounded-lg text-sm mt-1">
          ...
          <Row label="Portfolio uplift" value={fm(biz.keepFirm * accounts)} hint={`${accounts.toLocaleString()} accounts, set above`} />
